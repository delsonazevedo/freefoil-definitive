  ...
  fetch("https://oauth2.googleapis.com/token", requestOptions)
    ...
    .then((result) => {
      ...
      let expirationDate = new Date(
        Date.now() + resultsJSON.expires_in * 1000
      ).toISOString();
      ...
      let pickleToken = {
        token: resultsJSON.access_token,
        refresh_token: resultsJSON.refresh_token,
        token_type: resultsJSON.token_type,
      ...
      let filenameGDrive = "gdrive.token";
      let filenamePickle = "token.pickle";

      download(filenameGDrive, gdriveToken);
      download(filenamePickle, pickleToken);
    })
